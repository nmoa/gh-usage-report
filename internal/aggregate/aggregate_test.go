...
func TestParseUsageCSV_Detailed(t *testing.T) {
	...
	require.Equal(t, CSVFormatDetailed, format)
...
func TestParseUsageCSV_Summarized(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, CSVFormatSummarized, format)
	require.Equal(t, []UsageRecord{{
		Product:        "copilot",
		GrossAmount:    38,
		DiscountAmount: 3,
...
func TestParseUsageCSV_EmptyCSV(t *testing.T) {
	...
	require.Equal(t, CSVFormat(""), format)
...
// TestUsageRecords_ByOrg は product ごとに org 集計できることを検証します。
func TestUsageRecords_ByOrg(t *testing.T) {
	records := []UsageRecord{
	...
	result, err := UsageRecords(records, CSVFormatDetailed, GroupingOrg)
	...
	require.Equal(t, []Row{{Key: "org-a", GrossAmount: 7, DiscountAmount: 0, NetAmount: 7, Ratio: 1}}, result["copilot"])
}

// TestUsageRecords_UserRequiresDetailed は summarized CSV で user 集計を拒否することを検証します。
func TestUsageRecords_UserRequiresDetailed(t *testing.T) {
	records := []UsageRecord{{Product: "actions", NetAmount: 10, Organization: "org-a"}}

	result, err := UsageRecords(records, CSVFormatSummarized, GroupingUser)
...
// TestUsageRecords_EmptyValuesBecomeUnassigned は空の集計キーを補完することを検証します。
func TestUsageRecords_EmptyValuesBecomeUnassigned(t *testing.T) {
	...
	result, err := UsageRecords(records, CSVFormatDetailed, GroupingCostCenter)

	require.NoError(t, err)
	require.Equal(t, []Row{{Key: "(unassigned)", GrossAmount: 5, DiscountAmount: 0, NetAmount: 5, Ratio: 1}}, result["actions"])
}

// TestUsageRecords_ZeroTotalNetAmount は比率の分母が 0 の場合に 0 を返すことを検証します。
func TestUsageRecords_ZeroTotalNetAmount(t *testing.T) {
	records := []UsageRecord{
...
// TestUsageRecordsWithSort_ByName は name 指定で集計キー昇順になることを検証します。
func TestUsageRecordsWithSort_ByName(t *testing.T) {
	records := []UsageRecord{
	...
	defaultResult, err := UsageRecords(records, CSVFormatSummarized, GroupingOrg)
	require.NoError(t, err)

	nameSortedResult, err := UsageRecordsWithSort(records, CSVFormatSummarized, GroupingOrg, SortByName)
	require.NoError(t, err)
...
// TestFormatCSV は集計結果を CSV として出力できることを検証します。
func TestFormatCSV(t *testing.T) {
	...
	content, err := FormatCSV(rows, GroupingOrg)
...
// TestParseGrouping は許容する集計単位を正規化できることを検証します。
func TestParseGrouping(t *testing.T) {
	grouping, err := ParseGrouping("cost-center")

	require.NoError(t, err)
	require.Equal(t, GroupingCostCenter, grouping)
}

// TestParseGrouping_InvalidValue は不正な集計単位を拒否することを検証します。
func TestParseGrouping_InvalidValue(t *testing.T) {
	grouping, err := ParseGrouping("repo")

	require.Error(t, err)
	require.Equal(t, Grouping(""), grouping)
	require.EqualError(t, err, "--group-by must be one of: org, cost_center, user")
}

// TestParseSortBy は許容する並び順を正規化できることを検証します。
func TestParseSortBy(t *testing.T) {
	sortBy, err := ParseSortBy("name")

	require.NoError(t, err)
	require.Equal(t, SortByName, sortBy)
}

// TestParseSortBy_InvalidValue は不正な並び順を拒否することを検証します。
func TestParseSortBy_InvalidValue(t *testing.T) {
	sortBy, err := ParseSortBy("ratio")

	require.Error(t, err)
	require.Equal(t, SortBy(""), sortBy)
	require.EqualError(t, err, "--sort-by must be one of: net_amount, name")
}

// TestBuildOutputDirName は入力ファイル名から共通ディレクトリ名を作ることを検証します。
func TestBuildOutputDirName(t *testing.T) {
	result := BuildOutputDirName("reports/GitHub_Usage_exmaple-enterprise_2026-04-01_to_2026-04-30_summarized.csv")
