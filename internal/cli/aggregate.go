...
import (
	...
	"github.com/nmoa/gh-usage-report/internal/aggregate"
)
...
func newAggregateCmd(deps aggregateCommandDependencies) *cobra.Command {
	...
	cmd.Flags().String("sort-by", string(aggregate.SortByNetAmount), "Row ordering: net_amount or name")
...
func runAggregateCommand(cmd *cobra.Command, deps aggregateCommandDependencies) error {
	...
	records, format, err := aggregate.ParseUsageCSV(inputFile)
	if err != nil {
		return err
	}
	if len(records) == 0 {
	...
	aggregatedRows, err := aggregate.UsageRecordsWithSort(records, format, options.grouping, options.sortBy)
	if err != nil {
		return err
	}

	outputDir := filepath.Join(options.outputDir, aggregate.BuildOutputDirName(options.inputPath))
	...
	for _, product := range products {
		content, err := aggregate.FormatCSV(aggregatedRows[product], options.grouping)
		if err != nil {
			return err
		}

		outputPath := filepath.Join(outputDir, aggregate.BuildOutputFileName(product, options.grouping))
...
func readAggregateCommandOptions(cmd *cobra.Command) (aggregateCommandOptions, error) {
	...
	grouping, err := aggregate.ParseGrouping(groupByValue)
	if err != nil {
		return aggregateCommandOptions{}, err
	}
	...
	sortBy, err := aggregate.ParseSortBy(sortByValue)
	if err != nil {
		return aggregateCommandOptions{}, err
	}
