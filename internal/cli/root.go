// Package cli は gh usage-report CLI のコマンド定義と実行フローを提供します。
package cli
...
import (
	...
	"github.com/nmoa/gh-usage-report/internal/api"
	"github.com/nmoa/gh-usage-report/internal/billing"
	"github.com/nmoa/gh-usage-report/internal/download"
)
...
type reportClient interface {
	CreateReport(ctx context.Context, enterprise string, req api.CreateReportRequest) (*api.ReportExport, error)
	GetReport(ctx context.Context, enterprise string, reportID string) (*api.ReportExport, error)
}
...
type commandDependencies struct {
	newRESTClient func(authToken string, logOutput io.Writer) (*ghapi.RESTClient, error)
...
func newDefaultDependencies() commandDependencies {
	return commandDependencies{
		newRESTClient: func(authToken string, logOutput io.Writer) (*ghapi.RESTClient, error) {
			return ghapi.NewRESTClient(ghapi.ClientOptions{
				AuthToken: authToken,
				Log:       logOutput,
			})
		},
		downloader:   download.NewHTTPDownloader(nil),
...
// NewRootCmd は本番実行用の CLI エントリーポイントを構築します。
func NewRootCmd() *cobra.Command {
	return newRootCmd(newDefaultDependencies())
}
...
func runRootCommand(cmd *cobra.Command, deps commandDependencies) error {
	...
	return run(cmd.Context(), deps, logger, api.NewOctokit(restClient), options)
}
...
func readCommandOptions(cmd *cobra.Command) (commandOptions, error) {
	...
	return commandOptions{
		...
		billingCycle: billing.NewBillingCycle(billing.InputCycle{
			Year:         year,
			Month:        month,
...
func run(ctx context.Context, deps commandDependencies, logger *log.Logger, rc reportClient, options commandOptions) error {
	...
	for _, currentReportType := range reportTypes {
		if err := createAndSaveReport(ctx, deps, logger, rc, options, currentReportType); err != nil {
			return err
		}
	}
...
func createAndSaveReport(ctx context.Context, deps commandDependencies, logger *log.Logger, rc reportClient, options commandOptions, reportType string) error {
	logger.Printf("Generating %s report", reportType)

	report, err := rc.CreateReport(ctx, options.enterprise, api.CreateReportRequest{
		ReportType: reportType,
	...
	completedReport, err := waitForCompletion(ctx, rc, options.enterprise, report.ID, options.timeout, deps.pollInterval, logger)
	if err != nil {
...
func waitForCompletion(ctx context.Context, rc reportClient, enterprise string, reportID string, timeout time.Duration, pollInterval time.Duration, logger *log.Logger) (*api.ReportExport, error) {
	...
	for {
		report, err := rc.GetReport(timeoutContext, enterprise, reportID)
		if err != nil {
...
func buildFilename(enterprise string, bc *billing.BillingCycle, reportType string, index int, total int) string {
	fileName := fmt.Sprintf("GitHub_Usage_%s_%s_%s", enterprise, bc.GetDateRangeAsString(), reportType)
	if total > 1 && index > 0 {
